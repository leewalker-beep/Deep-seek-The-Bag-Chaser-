...
describe('Building / LaborBuild First-Play Reproduction Audit', () => {
  it('TEST 1: Real First-Play Reproduction Across 100+ Background/Focus Variations & Starter Hustles', { timeout: 30000 }, () => {
    const focusOptions: ('capital' | 'clout' | 'aura' | 'balanced')[] = ['capital', 'clout', 'aura', 'balanced'];
    const multipliers = [0, 0.1, 0.5, 0.8, 1.0, 1.2, 2.0];
    const starterHustles = ['r_labor', 'r_delivery', 'r_scrap', 'street_eats', 'cleaning'];
    ...
    for (const hustleId of starterHustles) {
      for (const bg of BACKGROUNDS) {
        for (const focus of focusOptions) {
          for (const mult of multipliers) {
            ...
            const startingLevel = stateBefore.hustleLevels[hustleId] || 1;
            const playerTier = stateBefore.currentTier;
            const originBackground = bg.id;

            // FIRST PLAY of starter hustle
            const result = useGameStore.getState().executeHustle(hustleId, mult);
            ...
            if (isDead) {
              ...
              deathRecords.push({
                ...
                lastActionExecuted: stateAfter.lastExecutedHustleId || hustleId
              });
            }
          }
        }
