...
export const HallOfFame: React.FC<HallOfFameProps> = ({ onNewRun }) => {
  ...
  const handleShare = async (run: HallOfFameEntry | null) => {
    ...
    const shareText = `Bag Chaser Hall of Fame:
Best Run: ${run.tier} tier, Legacy Score: ${run.legacyScore.toLocaleString()}, Ending: ${run.ending}
Total Runs: ${entries.length} | Endings Unlocked: ${unlockedEndings.length}/16
Can you beat me?`;

    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Bag Chaser Hall of Fame',
          text: shareText,
  ...
  const renderBestRun = () => {
    ...
    return (
      ...
      >
        ...
        <div className="pt-4">
          <BaseButton variant="secondary" onClick={() => handleShare(bestRun)} className="w-full py-4 text-lg">
            {copied ? 'COPIED TO CLIPBOARD!' : 'SHARE THIS RUN'}
          </BaseButton>
        </div>
      </motion.div>
    );
  ...
  const renderAllRuns = () => {
    ...
    return (
      <div className="space-y-3">
        {entries.slice().reverse().map((run, index) => (
          <div
            key={run.runId}
          ...
          >
            ...
            >
              <div className="flex flex-col items-start">
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Run #{entries.length - index}</span>
              ...
              {expandedRunId === run.runId && (
                ...
                >
                  ...
                  <BaseButton
                    ...
                    className="w-full mt-3 py-2 text-[10px]"
                  >
                    SHARE RUN
                  </BaseButton>
                </motion.div>
          ...
          return (
            ...
            >
              ...
              <div className={`text-[10px] font-black uppercase tracking-tight mb-1 ${isUnlocked ? 'text-purple-400' : 'text-slate-600'}`}>
                {isUnlocked ? ending.title : 'Locked Ending'}
              </div>
              <p className="text-[9px] leading-tight text-slate-400">
                {isUnlocked ? ending.description : 'Unlock this ending through gameplay to reveal its secrets.'}
              </p>
            </div>
          );
        })}
