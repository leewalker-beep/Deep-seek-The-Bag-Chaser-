...
export const StrategicAdvisorModal: React.FC<StrategicAdvisorModalProps> = ({ onClose }) => {
  ...
  const [activePriorityFilter, setActivePriorityFilter] = useState<'ALL' | 'CRITICAL' | 'IMPORTANT' | 'OPPORTUNITIES' | 'INFO'>('ALL');
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5 no-scrollbar">
          ...
          <div className="flex bg-slate-950 p-1 rounded-xl border border-slate-800 overflow-x-auto no-scrollbar gap-1">
            {(['ALL', 'CRITICAL', 'IMPORTANT', 'OPPORTUNITIES', 'INFO'] as const).map(tab => (
              <button
                key={tab}
                onClick={() => setActivePriorityFilter(tab)}
                className={`flex-1 text-center py-1 px-1.5 text-[8px] font-black uppercase tracking-wider rounded-md transition-all whitespace-nowrap ${
