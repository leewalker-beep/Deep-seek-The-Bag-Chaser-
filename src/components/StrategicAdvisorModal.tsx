      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5 no-scrollbar">
          ...
          <div className="flex bg-slate-950 p-1 rounded-xl border border-slate-800 overflow-x-auto no-scrollbar gap-1">
            {(['ALL', 'CRITICAL', 'IMPORTANT', 'OPPORTUNITIES', 'INFO', 'REPUTATION', 'AMBITIONS', 'HISTORY'] as const).map(tab => (
              <button
                key={tab}
                onClick={() => {
              ...
              >
                {tab === 'AMBITIONS' ? '🎯 AMBITIONS' : tab === 'HISTORY' ? '📖 HISTORY' : tab === 'REPUTATION' ? '🏆 REPUTATION' : tab}
              </button>
            ))}
          </div>
          ...
          ) : activePriorityFilter === 'REPUTATION' ? (
            (() => {
