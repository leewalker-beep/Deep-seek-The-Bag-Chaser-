...
export const Scoreboard: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const [activeTab, setActiveTab] = useState<'career' | 'portfolio' | 'badges' | 'history' | 'biography' | 'reputation' | 'ledger' | 'empire' | 'achievements' | 'endings' | 'deaths'>('career');
  const [confirmingEnd, setConfirmingEnd] = useState(false);

  const { setPh, updatePl } = useGameStore();
  ...
  const getTabStatus = (tabName: string) => {
    ...
    if (['career', 'portfolio', 'badges', 'achievements', 'endings', 'deaths'].includes(tabName)) {
    ...
    <div className={`fixed inset-0 ${showTutorial ? 'z-[200]' : 'z-50'} flex items-center justify-center p-4 bg-slate-950/95 backdrop-blur-xl`}>
      ...
      >
        ...
        <div className="flex bg-slate-950/80 p-1 m-4 rounded-xl border border-slate-800/80 overflow-x-auto no-scrollbar gap-1">
          {['career', 'portfolio', 'badges', 'history', 'biography', 'reputation', 'ledger', 'empire', 'achievements', 'endings', 'deaths'].map((tab) => {
