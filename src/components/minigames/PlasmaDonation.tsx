...
export const PlasmaDonation: React.FC<PlasmaDonationProps> = ({ onComplete }) => {
  ...
  const handleRelease = (finalProgress?: number) => {
    ...
    // Sweet spot is 80-90
    let multiplier = 0.5;
    if (actualProgress >= 80 && actualProgress <= 90) multiplier = 3.0;
    else if (actualProgress >= 60 && actualProgress <= 95) multiplier = 1.5;
  ...
  return (
    <div className="bg-red-950/20 p-8 rounded-3xl border-4 border-red-900/50 shadow-2xl text-center max-w-sm w-full mx-auto flex flex-col items-center">
      ...
      <div className="relative w-24 h-64 bg-slate-900 rounded-full border-4 border-slate-800 p-1 mb-8 overflow-hidden">
        ...
        <motion.div
          ...
          className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-red-800 to-red-500 rounded-b-full z-10"
        />
