...
export const ScoopThePoop: React.FC<ScoopThePoopProps> = ({
  ...
  const [tiles, setTiles] = useState<Record<string, '💩' | '✨' | null>>({});
  ...
  const spawnRate = useMemo(() => Math.max(400, 1200 - (level - 1) * 200), [level]);
  ...
  const scoop = useCallback((key: string) => {
    if (!gameActive || tiles[key] !== '💩') return;
  ...
  useEffect(() => {
    if (!gameActive) {
      let base = 0.5;
      if (score >= 20) base = 3.5;
      else if (score >= 12) base = 2.0;
      else if (score >= 6) base = 1.0;
  ...
  return (
    <div className="bg-slate-950 p-6 rounded-3xl border-4 border-slate-900 shadow-2xl text-center max-w-sm w-full mx-auto relative overflow-hidden">
      <div className="mb-4">
        ...
        <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">TAP FAST — DON'T LET IT PILE UP</p>
      </div>
