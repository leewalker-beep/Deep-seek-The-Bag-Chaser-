...
export const SequenceRecall: React.FC<SequenceRecallProps> = ({
  ...
  const totalRounds = useMemo(() => Math.min(5, 3 + Math.floor(level / 2)), [level]);
  const baseSeqLength = useMemo(() => 2 + Math.floor(scaling * 0.4), [scaling]);

  const startRound = (r: number) => {
    const newSeq = Array.from({ length: r + baseSeqLength }, () => Math.floor(Math.random() * 4));
    setSequence(newSeq);
    setUserInput([]);
    setIsPlaying(true);
    setShowIndex(-1);
  ...
  useEffect(() => {
    if (isPlaying) {
      const displayInterval = Math.max(150, 600 * timerFactor);
  ...
  }, [isPlaying, showIndex, sequence, timerFactor]);
  ...
  const handleInput = (index: number) => {
    ...
    if (index !== sequence[userInput.length]) {
      ...
      let multiplier = 0.5;
      if (round >= 3) multiplier = 1.8;
      else if (round >= 2) multiplier = 1.2;
    ...
    if (nextInput.length === sequence.length) {
      if (round >= totalRounds) {
        ...
        setTimeout(() => onComplete(3.0 + scaling * 0.5), 1000);
      } else {
  ...
  return (
    <div className={`transition-colors duration-200 bg-slate-900 p-8 rounded-3xl border-4 shadow-2xl text-center max-w-sm w-full mx-auto ${
      ...
      <h2 className="text-2xl font-black text-blue-400 mb-2 uppercase italic tracking-tighter">{title} <span className="text-white text-xs">L{level}</span></h2>
      <div className="flex flex-col items-center gap-1 mb-8">
        ...
        <div className="flex gap-2">
            {[...Array(totalRounds)].map((_, i) => {
                const r = i + 1;
                return (
                    <div key={r} className={`w-8 h-1.5 rounded-full transition-colors duration-300 ${round > r ? 'bg-emerald-500' : round === r ? 'bg-blue-500 animate-pulse' : 'bg-slate-800'}`} />
                );
            })}
        </div>
      </div>
